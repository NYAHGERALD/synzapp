/**
 * Every country a person can pick when setting up a company.
 *
 * Generated from the system's own ICU data rather than typed out, because a
 * hand-written list of 251 countries acquires a misspelling nobody notices
 * until somebody from that country tries to sign up.
 *
 * Firebase phone sign-in works by SMS and is not limited to a short list of
 * countries, so this is the full ISO 3166-1 set rather than a subset. Codes
 * that are not countries — deprecated entries, and reserved ones like EU — are
 * left out.
 */
...
export function getCountryLabel(code: string): string {
  const match = ALL_COUNTRIES.find((country) => country.code === code.toUpperCase());
