...
const RCA_KNOWLEDGE_SYSTEM_PROMPT = [
  ...
  'Synzapp incident carry-over: connecting an Incident node to an Incident Details node copies the Incident\'s "Incident Description" into the Incident Details "What Happened?" field, so the account does not have to be retyped. It only fills that field when nobody has written in it — a field holding somebody\'s own words is left alone — and because "What Happened?" is also the node\'s title, the Incident Details node is renamed to match, exactly as it would be if the field were typed by hand. To change it afterwards, edit "What Happened?" on the Incident Details node; editing the Incident\'s description later does not copy again.',
